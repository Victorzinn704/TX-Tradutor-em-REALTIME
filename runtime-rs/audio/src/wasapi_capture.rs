...
impl MicCapture {
    ...
    pub fn new(device_index: u32, sample_rate: u32, channels: u16) -> Self {
        Self {
            source_id: format!("mic:{device_index}"),
            device_index,
            sample_rate,
            channels,
    ...
    pub fn default_device() -> Result<Self, CaptureError> {
        crate::device_enum::find_default_mic().map(|device| Self::from_device_info(&device))
    }
}
...
fn resolve_device_index(device_index: u32) -> Result<wasapi::Device, CaptureError> {
    let collection = wasapi::DeviceCollection::new(&Direction::Capture)
...
fn resolve_device_info(device_index: u32) -> Result<crate::device_enum::DeviceInfo, CaptureError> {
    crate::device_enum::enumerate_devices()?
        ...
        .ok_or_else(|| CaptureError::DeviceNotFound(format!("mic:{device_index}")))
}
        ...
        match ready_rx.recv() {
            ...
            Err(error) => {
                ...
                Err(CaptureError::StreamOpen(format!(
                    "failed to initialize microphone capture: {error}"
                )))
            }
        }
    }
...
impl AudioCapture for MicCapture {
    ...
    fn start(&mut self, _callback: AudioCallback) -> Result<(), CaptureError> {
        Err(CaptureError::Wasapi(
            "microphone capture is only available on Windows".to_string(),
        ))
    }
